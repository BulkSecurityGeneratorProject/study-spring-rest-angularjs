(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('restangularjsApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-restangularjsApp-error');
                var entityKey = httpResponse.headers('X-restangularjsApp-params');
                if (errorHeader) {
